...
const useCharacter = () => {
    ...
    const defaultCharacter = { name: '', birth_year: '', height: '', url: '', films: [], starships: [], vehicles: [], planet: { name: '', climate: '', terrain: '' } }
    const [characterData, setCharacterData] = useState<Character>(defaultCharacter);

    useEffect(() => {
        // Populating character data from sessionStorage if present (client side caching)
        const existingCharacterData: Character = ss.getWithExpiry(`character-${characterId}`) || null;
    ...
    useEffect(() => {
        if (characterData?.name) {
            const actualCharacterName: string = characterData?.name.replace(/ /g, '-').toLowerCase();
            if (characterName !== actualCharacterName) {
                navigate(`/character/${actualCharacterName}/${characterId}`, { replace: true });
            }
        }
    ...
    const handleRightArrow: (event: React.MouseEvent<HTMLElement>) => void = (e) => {
        const characterId = e.currentTarget.dataset.characterid || '1';
        navigate(`/character/random/${Number(characterId) + 1}`)
    };
