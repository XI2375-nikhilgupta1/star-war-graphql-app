...
const CharacterPage: React.FC = () => {
    ...
    const { handleRightArrow, handleLeftArrow, showLoader, error, characterData, characterImage } = useCharacter();
    ...
    if (showLoader) {
        return (<div className="flex justify-center items-center h-screen character-page">
    ...
    return <div className="flex justify-center items-center h-screen character-page">
        <div className="character-container sm:w-full md:w-3/5 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 flex sm:flex-wrap md:flex-nowrap  gap-8 relative">
            {!hideLeftButton && <button data-characterId={characterId} onClick={handleLeftArrow} className="absolute left-icon carousel-btn shadow">&#8592;</button>}
            <button data-characterId={characterId} onClick={handleRightArrow} className="absolute right-icon carousel-btn shadow">&#8594;</button>
